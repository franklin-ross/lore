...
function arrowSizeFromConfig(): number {
  const v = vscode.workspace.getConfiguration("lore").get<number>("graph.arrowSize");
...
export class LoreGraphPanel {
  ...
  async refresh(): Promise<void> {
    ...
    try {
      payload = await client.sendRequest<GraphResponse>("lore/graph", { textDocument: td });
    } catch (err) {
  ...
  private setFocus(entity: string, source: string | undefined): void {
    if (entity === this.currentFocus && source === this.currentSource) return;
  ...
  async restore(panel: vscode.WebviewPanel, raw: unknown): Promise<void> {
    const state = (raw && typeof raw === "object") ? raw as { focus?: unknown; source?: unknown } : {};
  ...
  private ensurePanel(): void {
    ...
    const webviewRoot = vscode.Uri.joinPath(this.context.extensionUri, "out", "graph");
    ...
    switch (msg.type) {
      ...
      case "openEntity":
        if (msg.entity) {
          ...
          await vscode.commands.executeCommand("lore.openWiki", msg.entity);
        }
        return;
    }
  }
  ...
  private renderShell(): string {
    ...
    const mediaUri = (rel: string): vscode.Uri =>
      webview.asWebviewUri(
        vscode.Uri.joinPath(this.context.extensionUri, "out", "graph", rel),
      );
